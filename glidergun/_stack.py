...
class Stack(Sam):
    ...
    def project(self, crs: int | CRS, resampling: Resampling | ResamplingMethod = "nearest"):
...
@overload
def stack(
    data: Sequence[str], extent: tuple[float, float, float, float] | None = None, crs: int | CRS | None = None
) -> Stack:
...
def read_grids(dataset, extent: tuple[float, float, float, float] | None, crs: int | CRS | None) -> Iterator[Grid]:
