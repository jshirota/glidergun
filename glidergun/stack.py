...
def stack(
    data: Basemap | str,
    extent: BBox | Literal["#"] | None = (-179.999, -85.0511, 179.999, 85.0511),
    *,
    max_tiles: int = 10,
    ...
    if dataset.subdatasets:
        ...
        for index in dataset.indexes:
            with contextlib.suppress(Exception):
                yield from_dataset(dataset, extent, resample_by=resample_by, resampling=resampling, index=index)


def from_tile_service(
    url_template: Basemap | str,
    ...
    *,
    ...
    if url_template in BasemapUrl:
        url_template = BasemapUrl[url_template]

    if not extent:
        extent = (-179.999, -85.0511, 179.999, 85.0511)
