...
class Mosaic:
    def __init__(self, *files: str, blend: bool = False) -> None:
        ...
        if len(count_set) != 1:
            raise ValueError("Inconsistent number of bands")
        if len(crs_set) != 1:
            raise ValueError("Inconsistent CRS")
        ...
        for i, (f, e) in enumerate(self.files.items()):
            ...
            try:
                if e.intersects(extent):
                    yield grid(f, extent, index=index, resample_by=resample_by, resampling=resampling)
            except Exception:
                pass
    ...
    def tiles(
        ...
        for e in extent.tiles(width, height):
            g = self.clip(e)
            if not g:
                raise ValueError("Tile could not be clipped or is empty.")
            yield g
        ...
        *,
        ...
        try:
            if not index or isinstance(index, int):
                grids = self._read(extent, index=index or 1, resample_by=resample_by, resampling=resampling)
                return mosaic(*(g for g in grids if g), blend=self.blend)
            return stack([self.clip(extent, index=i, resample_by=resample_by, resampling=resampling) for i in index])
        except Exception:
            return None
