...
class ItemBase(PystacItem):
    def __init__(self, *args, **kwargs):
        ...
        self.fully_contains_search_extent: bool = None  # type: ignore
...
@overload
def search(
    collection: Literal["aster", "aster-l1t"],
    extent: BBox | Literal["#"],
...
@overload
def search(
    collection: Literal["landsat", "landsat-c2-l2"],
    extent: BBox | Literal["#"],
    query: dict | None = None,
    *,
    datetime: DateRange | None = None,
...
@overload
def search(
    collection: Literal["sentinel", "sentinel-2-l2a"],
    extent: BBox | Literal["#"],
    query: dict | None = None,
    *,
    datetime: DateRange | None = None,
...
def search(
    ...
    *,
    ...
    json = {
        ...
        "query": {"eo:cloud_cover": {"lt": max_cloud_cover_percent}} | (query or {})
        if max_cloud_cover_percent is not None
        else query or {},
    }
    ...
    for _ in range(4):
        ...
        if response.status_code != 504:
            break
        logging.warning(f"Search request failed with status code {response.status_code}.  Retrying...")

    response.raise_for_status()  # type: ignore

    features = []

    for feature in response.json()["features"]:  # type: ignore
        geometry = shape(feature["geometry"])
        fully_contains_search_extent = search_polygon.within(geometry)
        ...
        item.crs = to_crs
        item.fully_contains_search_extent = fully_contains_search_extent
...
def search_mosaic(
    ...
    items = search(collection, extent, url=url, limit=limit)
    if not items:
