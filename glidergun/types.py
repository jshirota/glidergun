...
class Extent:
    ...
    def __init__(self, xmin: float, ymin: float, xmax: float, ymax: float, crs: int | str | CRS = 4326) -> None:
    ...
    @staticmethod
    def from_active_map():
        """Creates an Extent from the active map in a supported GIS environment (ArcGIS Pro or QGIS)."""
        if "arcpy" in sys.modules:
            import arcpy  # type: ignore

            aprx = arcpy.mp.ArcGISProject("CURRENT")
            e = aprx.activeView.camera.getExtent()
            return Extent(e.XMin, e.YMin, e.XMax, e.YMax, e.spatialReference.exportToString())
        ...
        if "qgis.core" in sys.modules:
            ...
            e = canvas.extent()
            return Extent(
                e.xMinimum(), e.yMinimum(), e.xMaximum(), e.yMaximum(), canvas.mapSettings().destinationCrs().toWkt()
            )
