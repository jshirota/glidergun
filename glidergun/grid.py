...
def grid(  # type: ignore
    ...
    *,
    ...
    with warnings.catch_warnings():
        ...
        match data:
            case DatasetReader():
                return from_dataset(data, extent, index, resample_by, resampling or "nearest")
            case str():
                if data == "cop-dem-glo-30" or data == "cop-dem-glo-90":
                    ...
                    if extent.width * extent.height > 1000:
                        raise ValueError("Extent is too large for cop-dem-glo-30 or cop-dem-glo-90.")
                    return from_cop_dem(
                ...
                with rasterio.open(data) as dataset:
                    return from_dataset(dataset, extent, index, resample_by, resampling or "nearest")
            case bytes():
...
def from_cop_dem(
    collection: Literal["cop-dem-glo-30", "cop-dem-glo-90"],
    extent: BBox,
    limit: int,
    resample_by: float | None = None,
    ...
    return search_mosaic(
        planetary_computer_url, collection, "data", extent, limit, resample_by=resample_by, resampling=resampling
    )
