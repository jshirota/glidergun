...
class Grid(GridCore, Interpolation, Focal, Zonal):
    ...
    def save(
        self, file: str | MemoryFile, dtype: DataType | None = None, driver: str = ""
    ):
...
def grid(
    ...
    match data:
        case DatasetReader():
            return from_dataset(data, extent, crs, cell_size, index)
        case str():
            with rasterio.open(data) as dataset:
                return from_dataset(dataset, extent, crs, cell_size, index)
        case MemoryFile():
        ...
        case _:
            ...
            if isinstance(data, (int, float)):
                return from_constant(data, extent, get_crs(crs or 4326), cell_size)
...
def from_dataset(
    ...
    if extent:
        xmin, ymin, xmax, ymax = extent
        if crs:
            [xmin, xmax], [ymin, ymax], *_ = rasterio.warp.transform(
                crs, dataset.crs, [xmin, xmax], [ymin, ymax]
            )
